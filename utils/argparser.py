...
def common_args():
    ...
    parser.add_argument("--scene", type=str, default="scene0004", help="Scene ScanNet.")
    parser.add_argument("--prefix", type=str, default="run0", help="Scene ScanNet.")

    parser.add_argument(
        "--data-tgt-dir",
        type=str,
        default=DATA_TGT_DIRECTORY,
        help="Path to the directory containing the target dataset.",
    )
    parser.add_argument(
        "--data-tgt-train-list",
        type=str,
        default=DATA_TGT_TRAIN_LIST_PATH,
        help="Path to the file listing the images*GT labels in the target train dataset.",
    )
    parser.add_argument(
        "--data-tgt-test-list",
        type=str,
        default=DATA_TGT_TEST_LIST_PATH,
    ...
    parser.add_argument("--ds-rate", default=DS_RATE, type=int, help="The downsampling rate in kc calculation.")
    parser.add_argument(
        "--init-tgt-port",
        default=INIT_TGT_PORT,
        type=float,
        dest="init_tgt_port",
    ...
    parser.add_argument(
        "--src-port-step",
        default=SRC_PORT_STEP,
        type=float,
        dest="src_port_step",
        help="The portion step in source domain in every round of self-paced self-trained neural network",
    )
    parser.add_argument("--randseed", default=RANDSEED, type=int, help="The random seed to sample the source dataset.")
    ...
    parser.add_argument("--info-max-loss", action="store_true", help="Use the infomation maximization loss")
    parser.add_argument("--lambda-ent", default=1, type=float, help="Reg weight for entropy")
    parser.add_argument(
        "--lambda-div", default=0.0, type=float, help="Reg weight for diversity (Dont use this. Hurts perf)"
    )
